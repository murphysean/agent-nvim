...
function M.sampling_available()
  ...
  local ok_proto, protocol = pcall(require, "agent-nvim.mcp.protocol")
  if ok_sessions and ok_proto and #sessions.list() > 0 and protocol.client_supports("sampling") then
    return true
  end
  local cfg = require("agent-nvim").config or {}
...
function M.activate()
  ...
  vim.keymap.set("n", "<leader>ac", function()
    require("agent-nvim.autocomplete").complete()
  ...
  vim.keymap.set({ "n", "v" }, "<leader>ae", function()
    require("agent-nvim.assist").explain()
  end, { desc = "AI explain" })
  vim.keymap.set({ "n", "v" }, "<leader>af", function()
    require("agent-nvim.assist").fix()
  end, { desc = "AI fix diagnostics" })
  vim.keymap.set({ "n", "v" }, "<leader>ar", function()
    require("agent-nvim.assist").refactor()
  end, { desc = "AI refactor" })
  vim.keymap.set({ "n", "v" }, "<leader>av", function()
    require("agent-nvim.assist").review()
  end, { desc = "AI review" })

  -- Completion (blink + native)
  local completion = require("agent-nvim.completion")
  completion.register()
  completion.on_client_connected()

  vim.notify("[agent-nvim] AI features activated", vim.log.levels.DEBUG)
end
...
function M.deactivate()
  ...
  vim.notify("[agent-nvim] AI features deactivated", vim.log.levels.DEBUG)
end
