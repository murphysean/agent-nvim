local http = require("agent-nvim.http")
local json = require("agent-nvim.json")
local protocol = require("agent-nvim.mcp.protocol")
local registry = require("agent-nvim.mcp.registry")
local sessions = require("agent-nvim.sessions")
local events = require("agent-nvim.events")
local lockfile = require("agent-nvim.lockfile")
...
function M.start(host, port)
  ...
  require("agent-nvim.tools").register_all()
  require("agent-nvim.resources").register_all()
  require("agent-nvim.prompts").register_all()

  local completion = require("agent-nvim.mcp.completion")
  completion.reset()
  ...
  local version = require("agent-nvim").version or "unknown"
...
function M.stop()
  if server_handle then
    ...
    sessions.reset()
    pcall(function()
      require("agent-nvim.bridge").revoke_all()
    end)
    server_handle:close()
    server_handle = nil
    bound_port = nil
