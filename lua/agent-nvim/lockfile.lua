...
---
--- Each running nvim with agent-nvim writes a lockfile under
--- $XDG_RUNTIME_DIR/agent-nvim/<pid>.json so external clients (Claude Code,
...
local function lock_dir()
  ...
  if runtime and runtime ~= "" then
    return runtime .. "/agent-nvim"
  end
  return vim.fn.stdpath("state") .. "/agent-nvim"
end
