...
---
--- This module is registered dynamically by agent-nvim on startup — users
--- don't need to configure anything if blink.cmp is installed.

--- @class agent-nvim.BlinkSource : blink.cmp.Source
local source = {}

--- Namespace for virtual text indicators
local ns = vim.api.nvim_create_namespace("agent_nvim_completion")
...
local function sampling_available()
  local ok, sessions = pcall(require, "agent-nvim.sessions")
...
function source:execute(context, item, callback, default_implementation)
  ...
  local context_mod = require("agent-nvim.prompts.context")
  local templates = require("agent-nvim.prompts.template_engine")
