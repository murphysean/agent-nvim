...
---
--- The agent-nvim plugin spawns external agents (e.g., goose acp) as child
...
--- bin/agent-stdio-bridge.lua`, which proxies every JSON-RPC frame back into
--- this nvim instance via the regular --listen RPC socket.
---
--- This module exposes the global `_agent_nvim_bridge_dispatch(token, body)`
...
local security = require("agent-nvim.security")
...
function _G._agent_nvim_bridge_dispatch(token, body)
