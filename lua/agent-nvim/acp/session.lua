...
local Connection = require("agent-nvim.acp.connection")
...
---   include_bridge boolean (default true)   — pass agent-nvim stdio bridge in mcpServers
      ...
      if not servername or servername == "" then
        if self._opts.on_status then
          pcall(
            ...
            "warning",
            "v:servername is empty and HTTP server not running; agent-nvim bridge skipped"
          )
        end
      else
        local token = bridge.mint(self._opts.spawn_id or "acp-session")
        self._bridge_token = token
        local script = self._opts.plugin_dir .. "/bin/agent-stdio-bridge.lua"
        table.insert(servers, {
          name = "agent-nvim",
          command = vim.v.progpath,
          args = { "--headless", "-l", script },
          env = {
            { name = "AGENT_NVIM_TARGET", value = servername },
            { name = "AGENT_NVIM_AUTH", value = token },
          },
        })
      end
    end
...
function Session:_dispatch_inbound_request(method, params, respond)
  if method == "fs/read_text_file" then
    require("agent-nvim.acp.fs").read_text_file(params, respond)
    return
  end
  if method == "fs/write_text_file" then
    require("agent-nvim.acp.fs").write_text_file(params, respond)
    return
  end
  if method == "terminal/create" then
    require("agent-nvim.acp.terminal").create(params, respond)
    return
  end
  if method == "terminal/output" then
    require("agent-nvim.acp.terminal").output(params, respond)
    return
  end
  if method == "terminal/wait_for_exit" then
    require("agent-nvim.acp.terminal").wait_for_exit(params, respond)
    return
  end
  if method == "terminal/kill" then
    require("agent-nvim.acp.terminal").kill(params, respond)
    return
  end
  if method == "terminal/release" then
    require("agent-nvim.acp.terminal").release(params, respond)
    return
  end
