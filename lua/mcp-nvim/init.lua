...
function M.setup(opts)
  ...
  -- Verification only; the real chat UI lives in mcp-nvim.chat.
  vim.api.nvim_create_user_command("McpAcpTest", function(cmd_opts)
    ...
    local sess = AcpSession.new({
      ...
      include_bridge = true,
      client_info = { name = "mcp-nvim", version = M.version },
  ...
  vim.api.nvim_create_user_command("McpBridgeTest", function()
    ...
    local script = plugin_dir .. "/bin/mcp-stdio-bridge.lua"
    ...
    local frame = require("mcp-nvim.json").encode({
      jsonrpc = "2.0",
      id = 1,
      method = "initialize",
      params = {
  ...
  vim.api.nvim_create_autocmd("VimLeavePre", {
    group = vim.api.nvim_create_augroup("mcp_nvim_shutdown", { clear = true }),
    callback = function()
      pcall(function()
        require("mcp-nvim.chat").shutdown()
      end)
      pcall(M.stop)
    end,
  })
...
--- Used to locate bin/mcp-stdio-bridge.lua and prompts/templates.
function M.plugin_dir()
  -- debug.getinfo gives us the path of this file: <root>/lua/mcp-nvim/init.lua
  local source = debug.getinfo(1, "S").source
  if source:sub(1, 1) == "@" then
    source = source:sub(2)
  end
  -- Walk up: init.lua -> mcp-nvim -> lua -> <plugin-root>
