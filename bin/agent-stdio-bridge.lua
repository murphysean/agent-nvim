--- Stdio MCP bridge for agent-nvim.
---
--- This is a standalone script run as a child of an ACP agent (e.g. goose):
---     nvim --headless -l <plugin>/bin/agent-stdio-bridge.lua
---
...
--- nvim RPC socket pointed to by env AGENT_NVIM_TARGET, authenticated with
--- AGENT_NVIM_AUTH.
---
...
local target = os.getenv("AGENT_NVIM_TARGET")
local auth = os.getenv("AGENT_NVIM_AUTH")

if not target or target == "" then
  io.stderr:write("[agent-stdio-bridge] AGENT_NVIM_TARGET not set\n")
  os.exit(1)
end
if not auth or auth == "" then
  io.stderr:write("[agent-stdio-bridge] AGENT_NVIM_AUTH not set\n")
  os.exit(1)
end

local ok, ch = pcall(vim.fn.sockconnect, "pipe", target, { rpc = true })
if not ok or not ch or ch == 0 then
  io.stderr:write(string.format("[agent-stdio-bridge] cannot connect to %s: %s\n", target, tostring(ch)))
  os.exit(1)
end

io.stderr:write(string.format("[agent-stdio-bridge] connected to %s (channel %d)\n", target, ch))
...
local function write_frame(line)
  ...
  stdout:write(payload, function(err)
    if err then
      io.stderr:write("[agent-stdio-bridge] stdout write error: " .. tostring(err) .. "\n")
    end
  end)
end
...
local function dispatch(line)
  ...
  -- channel's own keepalive) keep working. The parent's _agent_nvim_bridge_dispatch
  ...
  local rok, response =
    pcall(vim.rpcrequest, ch, "nvim_exec_lua", "return _G._agent_nvim_bridge_dispatch(...)", { auth, line })
  if not rok then
    io.stderr:write("[agent-stdio-bridge] rpc error: " .. tostring(response) .. "\n")
...
stdin:read_start(function(err, chunk)
  if err then
    io.stderr:write("[agent-stdio-bridge] stdin error: " .. tostring(err) .. "\n")
    exiting = true
    return
  end
  if not chunk then
  ...
  if #buf > 16 * 1024 * 1024 then
    io.stderr:write("[agent-stdio-bridge] buffer overflow, dropping\n")
    buf = ""
    return
  end
  while true do
...
io.stderr:write("[agent-stdio-bridge] shutting down\n")
pcall(function()
  stdin:close()
end)
pcall(function()
